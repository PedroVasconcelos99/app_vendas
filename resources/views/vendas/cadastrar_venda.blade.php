<x-layout>
    <h1 class="my-5">Cadastrar Venda</h1>
    <a href="{{ route('vendas.index') }}" class="mb-2">Voltar</a>
    <div class="flex justify-center">
        <form id="cadastrar_venda" action="{{ route('vendas.store') }}" method="post" class="flex flex-col items-end">
            @csrf
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="date" name="data_venda" value="{{ old('data_venda') }}" class="border @error('data_venda') border-red-500 @enderror">
                @error('data_venda')
                    <p class="text-red-500">{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <textarea name="observacao" class="border @error('observacao') border-red-500 @enderror">{{ old('observacao') }}</textarea>
                @error('observacao')
                    <p class="text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="valor_total" id="valor_total" value="{{ old('valor_total') }}" class="border @error('valor_total') border-red-500 @enderror" step="0.01" min="0" readonly>
                @error('valor_total')
                    <p class="text-red-500">{{ $message }}</p>
                @enderror
            </div>
