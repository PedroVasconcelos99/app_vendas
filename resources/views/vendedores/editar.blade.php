<x-layout>
    <div class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-bold text-center w-full">Atualizar Vendedor</h1>
    ...
    <div class="flex justify-center">
        <form id="cadastrar_pessoa" action="{{ route('vendedores.update', $vendedor) }}" method="post" class="w-full max-w-lg bg-white p-8 rounded-lg shadow-md">
            @csrf
            @method('PUT')
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nome" value="{{ $vendedor->nome }}" class="border rounded w-full py-2 px-3 text-gray-700 @error('nome') border-red-500 @enderror">
                @error('nome')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="cpf" id="cpf" value="{{ $vendedor->cpf }}" class="border rounded w-full py-2 px-3 text-gray-700 @error('cpf') border-red-500 @enderror">
                @error('cpf')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
